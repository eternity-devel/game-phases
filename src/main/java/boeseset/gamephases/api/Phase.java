package boeseset.gamephases.api;
...
public class Phase {
    ...
    public Phase itemTag(String tagId, boolean restrictRecipes) {
        TagKey<Item> tag = new TagKey(Registry.ITEM_KEY, new Identifier(tagId));
        ...
        /*
        if(items != null) {
            items.forEach(value -> item(value, restrictRecipes));
            items.
            blacklistedItemTags.add(tagId);
        } else {
            GamePhases.LOGGER.warn(String.format("Item tag '%s' was referenced in phase '%s', but the tag is not present!", tagId, id));
    ...
    public Phase blockTag(String tagId, Block replacement) {
        TagKey<Block> tag = new TagKey(Registry.BLOCK_KEY, new Identifier(tagId));
        Registry.BLOCK.forEach(item -> {
            if(item.getDefaultState().isIn(tag)) {
                block(item, replacement);
                blacklistedItemTags.add(tagId);
            } else {
    ...
    public Phase block(Block block, Block replacement) {
        ...
        BlockFib fib = BlockFib.builder(block, replacement)
                .withCondition(player -> !GamePhases.getPhaseData((PlayerEntity) player).has(this.id))
                .modifiesDrops()
                .build();
    ...
    public boolean hasUnlocked(PlayerEntity player) {
        return GamePhases.getPhaseData((PlayerEntity) player).has(this.id);
    }
